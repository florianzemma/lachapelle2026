...
const Programme = ({ slice }: ProgrammeProps) => {
  ...
  const hasTitle =
    slice.primary.programme_title && slice.primary.programme_title.length > 0;
  const hasDescription =
    slice.primary.programme_description &&
    slice.primary.programme_description.length > 0;
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-4xl px-6 lg:px-8">
        ...
        <article
          className={`overflow-hidden rounded-3xl bg-white shadow-xl transition-all duration-700 ease-out ${
        ...
        >
            ...
            <div className="border-b border-border p-6 text-center sm:p-8">
              {hasTitle && (
                <div className="prismic-content mb-3 font-heading text-2xl font-bold text-primary">
                  <PrismicRichText field={slice.primary.programme_title} />
                </div>
              )}
              {hasDescription && (
                <div className="prismic-content text-muted">
                  <PrismicRichText
                    field={slice.primary.programme_description}
                  />
                </div>
              )}
              {hasCta && (
          ...
          {slice.primary.programme_image &&
            slice.primary.programme_image.url && (
              <div className="relative w-full">
                <PrismicImage
                  field={slice.primary.programme_image}
